(function(){
  ...
  if(calcRoot){
    ...
    function buildJourney(){
      ...
      groups.forEach(group => {
        ...
        panel.innerHTML = `
          ...
          <div class="level-grid unlock-level-grid guided-level-grid slider-level-grid">${levelCards}</div>
        `;
    ...
    function setGroupState(key, unlocked, activeText){
      ...
      if(copy && group) copy.textContent = unlocked ? group.desc : (key === 'elite' ? group.desc : 'Kiraan tambahan akan terbuka selepas milestone dicapai.');
